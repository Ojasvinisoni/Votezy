...
function loadCandidates() {
  fetch("http://localhost:8081/api/candidate")
    .then((res) => res.json())
    .then((data) => {
...
function searchCandidate() {
  ...
  fetch(`http://localhost:8081/api/candidate/${candidateId}`)
    .then((res) => res.json())
    .then((candidate) => {
      Swal.fire({
...
function updateCandidate(candidateId, newName, newParty) {
  fetch(`http://localhost:8081/api/candidate/update/${candidateId}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name: newName, party: newParty }),
  })
...
function deleteCandidate(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      fetch(`http://localhost:8081/api/candidate/delete/${id}`, {
        method: "DELETE",
      })
        .then(() => {
